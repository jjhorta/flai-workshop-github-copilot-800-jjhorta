...
Check if octofit-tracker/backend/requirements.txt exists
Verify it contains the exact string Django==4.1
Check if the file has been committed and pushed to the build-octofit-app branch
Common fixes:
...
Verify Django project octofit_tracker was created in octofit-tracker/backend/
Check if settings.py has MongoDB configuration with djongo
Verify populate_db.py management command exists
Confirm the database name is octofit_db in settings
Check if all files are committed to build-octofit-app branch
Common fixes:
...
Check if codespace URL is configured in settings.py ALLOWED_HOSTS
Verify urls.py has REST API endpoints with codespace URL format
Confirm REST API endpoints are returning correct URLs in api_root
Check if both files are committed to build-octofit-app branch
Common fixes:
...
What GitHub Actions checks for: Five React component files must each contain -8000.app.github.dev/api/[component]/:
...
Verify React app was created in octofit-tracker/frontend/ directory
Check if all 5 component files exist in src/components/
Verify each component has fetch() calls to the correct API endpoint
Confirm the codespace URL format is used with the correct component path
Check if all files are committed to build-octofit-app branch
Common fixes:
...
Commit all changes to build-octofit-app branch
Execution Process
Identify the step from the user's input (must be 2, 3, 4, or 5)
Read the relevant workflow file from .github/workflows/[step-number]-*.yml to understand exact validation criteria
Check all required files against the validation criteria
Diagnose the issue by comparing what exists vs. what's expected
Apply fixes based on the common fixes and self-healing actions for that step
Verify the fix by re-checking files against validation criteria
Commit and push changes to the build-octofit-app branch if fixes were applied
Report back with:
What was wrong
What was fixed
